...
The RAG retrieval latency improvement was the engineering team's work, not mine. {% metric "reranker_batching_share" %}: collapsing sequential cross-encoder inference calls into one batched pass accounts for the overwhelming majority of it. That fix held until it didn't: a later, unrelated push to widen retrieval for quality reasons pulled in more candidates than the reranker had ever had to score, and batching on CPU stopped being enough to keep it off the critical path. The June 2026 release removed the reranker entirely rather than re-optimising it again, and that was likewise the engineering team's decision and implementation. My part was the measurement infrastructure that attributed both changes. Two changes shipped together in that release, so the end-to-end improvement cannot be attributed to either alone.
