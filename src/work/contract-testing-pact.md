...
| Option | Why not |
| --- | --- |
| OpenAPI schema diff alone | Cannot express a divergence conditional on the response's own content. It never issues a request, so it finds only what the schema language can say. Kept as a second layer — not as the answer. |
...
Consumer-driven contracts with Pact, a self-hosted broker, and `can-i-deploy` wired in as a real deployment gate — with the OpenAPI diff engine kept alongside rather than replaced by it.
