...
- Four limits turned out to be structural rather than tunable, so I migrated. Then four years later I retired the last corner, {% metric "robot_retirement_ported" %}, with a parallel-run cycle proving zero divergence first.
...
**Removing work rather than speeding it up.** {% metric "ui_scenarios_to_api" %} did not need browser-level verification at all. They asserted on data, not on rendering, and had been UI tests only because the UI suite was where tests lived. Some of what looks like a speed gain below is actually work removed from the wrong layer entirely.
...
- **The visual regression layer is a trade I would make again and would not extend.** {% metric "visual_regression" %}, at roughly 200–400ms per scenario. That ratio only holds at tight tolerance on a small, chosen set of screens.
